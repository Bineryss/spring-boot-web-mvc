...
public class AuthenticateWithUserDetailsServiceTest {
    @Autowired
    MockMvc mockMvc;
    @Autowired
    ...
    void testLoginSuccess() throws Exception {
        var testUser = new NewsUser("testuser", passwordEncoder.encode("pass"),
                "first", "last", LocalDate.now());
        Mockito.when(newsUserRepository.findByUsername("testuser")).thenReturn(testUser);
        mockMvc.perform(formLogin().user("testuser").password("pass"))
